...
def create_gantt_chart(df: pd.DataFrame) -> go.Figure:
    ...
    df_gantt['marker_start_time'] = df_gantt.groupby('marker_name')['start_time'].transform('min')

    # 2. Apply the lane assignment algorithm to each marker group
    df_gantt = df_gantt.groupby('marker_name', group_keys=False).apply(_assign_gantt_lanes)
...
def create_flame_graph(df: pd.DataFrame) -> go.Figure:
    ...
    fig = px.icicle(
        df_flame, ids='id', parents='parent', names='label', values='duration_ms',
        title='Performance Timing Flame Graph (Hierarchy View)', custom_data=['id'],
        color='duration_ms', color_continuous_scale='RdBu_r'
    )
    fig.update_layout(template=PLOTLY_THEME, height=chart_height, margin=dict(t=50, l=25, r=25, b=25))
    fig.update_traces(hovertemplate=None, hoverinfo='none')
    return fig